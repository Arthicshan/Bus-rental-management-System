...
const Lost = () => {
  ...
  const LOST_ITEMS_STORAGE_KEY = 'ahrthitravels_lost_items';
  ...
  const clearLocalStorage = () => {
    if (window.confirm('Are you sure you want to clear all local data? This cannot be undone.')) {
      ...
      localStorage.removeItem('ahrthitravels_lost_items');
      localStorage.removeItem('ahrthitravels_feedbacks');
      ...
      <div className="flex items-center">
        ...
        <Link to="/" className="mr-16 flex items-center space-x-3">
          <div className="relative">
            <div className="bg-gradient-to-r from-gray-800 to-black p-2 rounded-xl shadow-lg">
      ...
      <div className="flex items-center space-x-6 ml-auto">
        ...
        {user && (
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-gray-500 to-gray-700 flex items-center justify-center text-white font-semibold">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <div className="pt-20 px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              <button
                ...
                className="flex items-center space-x-2 px-4 py-2 bg-gray-800 hover:bg-black text-white rounded-lg transition-colors duration-200 group"
              >
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className="flex items-center px-6 py-2 bg-gradient-to-r from-gray-800 to-black hover:from-gray-700 hover:to-gray-900 text-white rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg"
              >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredItems.length === 0 ? (
              <div className="col-span-full bg-gradient-to-br from-white to-gray-50 rounded-2xl p-12 text-center border border-gray-200 shadow-lg">
                ...
                <button
                  onClick={() => setShowReportForm(true)}
                  className="px-6 py-3 bg-gradient-to-r from-gray-800 to-black text-white rounded-xl font-semibold hover:from-gray-700 hover:to-gray-900 transition-all duration-300 shadow-lg"
                >
                  Report First Item
                </button>
              </div>
              ...
              filteredItems.map((item) => (
                <div key={item._id} data-item-id={item._id} className="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-6 border border-gray-200 hover:border-gray-300 transition-all duration-300 relative shadow-lg hover:shadow-xl">
                  ...
                  <div className="mt-12"> {/* Added margin top to avoid overlap */}
                    <div className="mb-4">
                      <div className="flex items-center mb-1">
                        ...
                        {item.adminReply && item.adminReply.trim() !== '' && !item.deliveryStatus && (
                          <span className={`ml-2 text-white px-2 py-1 rounded-full text-xs font-bold animate-pulse ${item.status === 'Found' ? 'bg-green-500' :
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      {item.deliveryStatus && (
                        <div className="flex items-center text-sm text-slate-700">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${item.deliveryStatus === 'Delivered' ? 'bg-green-100 text-green-800' :
                    ...
                    {item.adminReply && item.adminReply.trim() !== '' && (
                      <div className={`rounded-xl p-4 mb-4 shadow-lg relative overflow-hidden ${item.status === 'Found' ? 'bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300' :
                        ...
                        <div className={`absolute inset-0 animate-pulse ${item.status === 'Found' ? 'bg-gradient-to-r from-green-100/50 to-emerald-100/50' :
                        ...
                        <div className="relative z-10">
                          <div className="flex items-start">
                            <div className={`p-2 rounded-full mr-3 flex-shrink-0 animate-bounce shadow-lg ${item.status === 'Found' ? 'bg-green-500' :
                              item.status === 'Not Found' ? 'bg-red-500' :
                                item.status === 'Claimed' ? 'bg-blue-500' :
                                  item.status === 'Returned' ? 'bg-purple-500' :
                                    'bg-green-500'
                            ...
                            <div className="flex-1">
                              <div className="flex items-center justify-between mb-2">
                                <p className={`text-sm font-bold flex items-center ${item.status === 'Found' ? 'text-green-800' :
                                  item.status === 'Not Found' ? 'text-red-800' :
                                    item.status === 'Claimed' ? 'text-blue-800' :
                                      item.status === 'Returned' ? 'text-purple-800' :
                                        'text-green-800'
                                  }`}>
                                  <span className={`px-2 py-1 rounded-full text-xs mr-2 animate-pulse shadow-md ${item.status === 'Found' ? 'bg-green-500 text-white' :
                                ...
                                {item.repliedBy && (
                                  <span className={`text-xs px-2 py-1 rounded-full shadow-sm ${item.status === 'Found' ? 'text-green-600 bg-green-100' :
                              ...
                              <div className={`rounded-lg p-3 border shadow-sm ${item.status === 'Found' ? 'bg-white border-green-200' :
                      ...
                      {user?.role === 'admin' ? (
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            className="flex-1 px-3 py-2 bg-gray-800 hover:bg-black text-white rounded-lg transition-colors text-sm flex items-center justify-center shadow-sm"
                          >
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            {canEditItem(item) && (
                              <button
                                onClick={() => handleEditItem(item)}
                                className="flex items-center px-3 py-2 bg-gray-800 hover:bg-black text-white rounded-lg transition-colors text-sm shadow-sm"
                              >
        ...
        {showReportForm && (
          <div className="fixed inset-0 bg-gray-100 bg-opacity-90 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl p-6 w-full max-w-lg border border-gray-200 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="p-2 hover:bg-gray-50 rounded-lg text-slate-400 hover:text-slate-600 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmitReport} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-gray-500 shadow-sm"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-gray-500 resize-none shadow-sm"
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-gray-500 shadow-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2 bg-white border rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 shadow-sm ${validationErrors.busNumber
                ...
                <div className="flex space-x-3 pt-3">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="flex-1 px-4 py-3 bg-gradient-to-r from-gray-800 to-black hover:from-gray-700 hover:to-gray-900 disabled:from-slate-400 disabled:to-slate-500 text-white rounded-lg font-medium transition-all duration-300 disabled:cursor-not-allowed shadow-lg"
                  >
              ...
              <form onSubmit={handleSubmitEdit} className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-gray-500 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                    placeholder="e.g., Black backpack, Red umbrella"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    disabled={user?.role !== 'admin' && !canEditItem(editingItem)}
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-gray-500 resize-none disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                    placeholder="Describe the item in detail..."
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    max={new Date().toISOString().split('T')[0]}
                    disabled={user?.role !== 'admin' && !canEditItem(editingItem)}
                    className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-gray-500 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleBusNumberChange(e.target.value)}
                    disabled={user?.role !== 'admin' && !canEditItem(editingItem)}
                    className={`w-full px-3 py-2 bg-white border rounded-lg text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm ${validationErrors.busNumber
                      ? 'border-red-500 focus:ring-red-500'
                      : 'border-gray-200 focus:ring-gray-500'
                      }`}
                    placeholder="e.g., NT7657"
                ...
                <div className="flex space-x-3 pt-3">
                  ...
                  <button
                    ...
                    className="flex-1 px-4 py-3 bg-gradient-to-r from-gray-800 to-black hover:from-gray-700 hover:to-gray-900 text-white rounded-lg font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:from-slate-400 disabled:to-slate-500 shadow-lg"
                  >
